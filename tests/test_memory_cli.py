...
def test_memory_cli_index_removes_stale_records(tmp_path: Path):
    ...
    memory_path.write_text(
        '{"record_id":"old-record","mission_id":"m1","record_type":"mission_outcome","content":{"command":"旧任务","status":"succeeded"},"created_at":"2026-06-11T00:00:00+00:00"}\n',
        encoding="utf-8",
    )
    assert memory_cli_main(["index", "--memory-path", str(memory_path), "--index-path", str(index_path)]) == 0
    ...
    memory_path.write_text(
        '{"record_id":"new-record","mission_id":"m2","record_type":"mission_outcome","content":{"command":"新任务","status":"succeeded"},"created_at":"2026-06-11T01:00:00+00:00"}\n',
        encoding="utf-8",
    )
    assert memory_cli_main(["index", "--memory-path", str(memory_path), "--index-path", str(index_path)]) == 0
    ...
    assert memory_cli_main(["eval", "--index-path", str(index_path), "--fixture", str(fixture_path), "--threshold", "1.0"]) == 2
