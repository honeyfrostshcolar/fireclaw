...
# --- Production lifecycle projection wiring tests (Task 1) ---

def test_mission_events_updates_subagent_registry_on_terminal_robot_event(tmp_path):
    """mission_events() should route terminal robot events into SubagentRegistry
    when MissionAgent has subagent_registry configured."""
    registry = RobotRegistry([
        RobotRegistryEntry(robot_id="robot-1", base_url="http://robot-1.local:8765"),
    ])
    client = FakeSubagentClient()
    ...
    # Pre-create a subagent run so mark_terminal has something to update
    ...
    # Set up terminal robot event
    client.events_by_entry[("robot-1", task_id)] = [
        {"type": "task.completed", "task_id": task_id, "timestamp": "2026-06-08T12:00:02Z", "payload": {}},
    ]

    result = mission.mission_events(mission_id)

    assert result["event_count"] == 1
    # Verify SubagentRegistry was updated via MissionAgent path
    record = subagent_registry.get_by_child_task_id(task_id)
    assert record is not None
    assert record.status == "completed"


def test_mission_events_updates_task_registry_on_terminal_robot_event(tmp_path):
    """mission_events() should project terminal robot events into TaskRegistry
    when MissionAgent has task_registry configured."""
    registry = RobotRegistry([
        RobotRegistryEntry(robot_id="robot-1", base_url="http://robot-1.local:8765"),
    ])
    client = FakeSubagentClient()
    ...
    assert result["event_count"] == 1
    # Verify TaskRegistry was updated with terminal status
    records = task_registry.list_records()
    subtask_records = [r for r in records if r.child_session_id == task_id]
    assert len(subtask_records) == 1
    assert subtask_records[0].status == "completed"
