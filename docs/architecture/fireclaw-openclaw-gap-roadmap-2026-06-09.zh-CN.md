...
| Gateway protocol/control plane | `FireClawGateway` HTTP endpoints + `MissionGateway` + method scopes | 核心 HTTP 防护已补齐。两端共享 Host/Origin 校验、请求头/正文超时、正文早期 `413`、线程创建前总量/单 IP 连接预算、认证失败锁定、SSE 连接与订阅队列上限；仍缺 WebSocket/control channel 和 classified method 的动态扩展。 |
...
| Sandbox / process permissions | deployment Tool policy、Docker `ComputerSandbox`、legacy process Tool fail-closed loading、固定 runtime root、canonical workspace policy、bounded Docker lifecycle | 已补齐核心边界。`*.skill.json` 仅能在 simulation profile 中作为 `effect=process + requires_sandbox` 注册，文件先受限复制到 sandbox workspace，real/无镜像/策略拒绝均阻断。Mission/Robot Gateway 在固定 runtime root 内启动；workspace 经过危险路径、允许根、符号链接和 Docker bind 前复验。Docker 调用使用命名容器、全终态强制清理、流式有界输出和 image ID 固定/复验。更细粒度的第三方 Plugin 权限仍是后续工作。 |
...
| Realtime streaming | `StreamEvent`, `EventBus`, SSE endpoints, `TelemetryTracker`, cursor replay | 已实现 v1。已有 SSE 实时流、统一事件 schema、cursor-based reconnect、总量/单 IP 连接预算、有界订阅队列和慢客户端写超时；缺 WebSocket 支持。 |
